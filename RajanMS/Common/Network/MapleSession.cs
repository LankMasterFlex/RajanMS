...
namespace Common.Network
{
    public sealed class MapleSession
    {
        ...
        {
            byte[] riv = new byte[] { 0x52, 0x61, 0x6A, 0x61 }; //Raja
            byte[] siv = new byte[] { 0x6E, 0x52, 0x30, 0x58 }; //nR0X

            m_sendCipher = new MapleCipher(Constants.MajorVersion, siv, MapleCipher.TransformDirection.Encrypt);
            m_recvCipher = new MapleCipher(Constants.MajorVersion, riv, MapleCipher.TransformDirection.Decrypt);
